...
type OpenOltResourceMgr struct {
	...
	DeviceType string
	Address    string              // Host ip and port of the kv store
...
func NewResourceMgr(ctx context.Context, deviceID string, KVStoreAddress string, kvStoreType string, deviceType string, devInfo *openolt.DeviceInfo) *OpenOltResourceMgr {
	...
	logger.Debugf("Init new resource manager , host_port: %s, deviceid: %s", KVStoreAddress, deviceID)
	...
	Backend := kvStoreType
	ResourceMgr.KVStore = SetKVClient(Backend, ResourceMgr.Address,
		deviceID)
